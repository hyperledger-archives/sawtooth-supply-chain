...
impl SupplyChainPayload {
    pub fn new (payload: &[u8]) -> Result<Option<SupplyChainPayload>, ApplyError> {
        let payload: payload::SCPayload = match protobuf::parse_from_bytes(payload) {
            Ok(payload) => payload,
            Err(_) => return Err(ApplyError::InvalidTransaction(String::from("Cannot deserialize payload")))
        };
        ...
        let action = match supply_chain_action {
            payload::SCPayload_Action::CREATE_AGENT => {
                ...
                if create_agent.get_name() == "" {
                    return Err(ApplyError::InvalidTransaction(String::from("Agent name cannot be an empty string")))
                }
            ...
            payload::SCPayload_Action::CREATE_RECORD => {
                ...
                if create_record.get_record_id() == "" {
                    return Err(ApplyError::InvalidTransaction(String::from("Record id cannot be empty string")))
                }
                Action::CreateRecord(create_record.clone())
            }
            payload::SCPayload_Action::FINALIZE_RECORD => Action::FinalizeRecord(payload.get_finalize_record().clone()),
            payload::SCPayload_Action::CREATE_RECORD_TYPE => {
                ...
                if create_record_type.get_name() == "" {
                    return Err(ApplyError::InvalidTransaction(String::from("Record Type name cannot be an empty string")))
                };
                let properties = create_record_type.get_properties();
                if properties.len() == 0 {
                    return Err(ApplyError::InvalidTransaction(String::from("Record type must have at least one property")))
                }
                for prop in properties {
                    if prop.name == "" {
                        return Err(ApplyError::InvalidTransaction(String::from("Property name cannot be an empty string")))
                    }
                }

                Action::CreateRecordType(create_record_type.clone())
            }
            payload::SCPayload_Action::UPDATE_PROPERTIES => Action::UpdateProperties(payload.get_update_properties().clone()),
            payload::SCPayload_Action::CREATE_PROPOSAL => Action::CreateProposal(payload.get_create_proposal().clone()),
            payload::SCPayload_Action::ANSWER_PROPOSAL => Action::AnswerProposal(payload.get_answer_proposal().clone()),
            payload::SCPayload_Action::REVOKE_REPORTER => Action::RevokeReporter(payload.get_revoke_reporter().clone()),
        };
        let timestamp = match payload.get_timestamp() {
            0 => return Err(ApplyError::InvalidTransaction(String::from("Timestamp is not set"))),
            x => x
        };
...
impl<'a> SupplyChainState<'a> {
    ...
    pub fn get_record(&mut self, record_id: &str) -> Result<Option<record::Record>, ApplyError> {
        ...
        match d {
            Some(packed) => {
                let records: record::RecordContainer = match protobuf::parse_from_bytes(packed.as_slice()) {
                    Ok(records) => records,
                    Err(_) => return Err(ApplyError::InternalError(String::from("Cannot deserialize record container")))
                };
    ...
    pub fn set_record(&mut self, record_id: &str, record: record::Record) -> Result<(), ApplyError> {
        let address = make_record_address(record_id);
        let d = self.context.get_state(&address)?;
        ...
        let serialized = match record_container.write_to_bytes() {
            ...
            Err(_) => return Err(ApplyError::InternalError(String::from("Cannot serialize record container")))
        };
        self.context.set_state(&address, serialized.as_ref())
            .map_err(|err|ApplyError::InternalError(format!("{}", err)))?;
        Ok(())
    }

    pub fn get_record_type(&mut self, type_name: &str) -> Result<Option<record::RecordType>, ApplyError> {
        let address = make_record_type_address(type_name);
        let d = self.context.get_state(&address)?;
        match d {
            Some(packed) => {
                let record_types: record::RecordTypeContainer = match protobuf::parse_from_bytes(packed.as_slice()) {
                    Ok(record_types) => record_types,
                    Err(_) => return Err(ApplyError::InternalError(String::from("Cannot deserialize record type container")))
                };
    ...
    pub fn set_record_type(&mut self, type_name: &str, record_type: record::RecordType) -> Result<(), ApplyError> {
        let address = make_record_type_address(type_name);
        let d = self.context.get_state(&address)?;
        ...
        let serialized = match record_types.write_to_bytes() {
            Ok(serialized) => serialized,
            Err(_) => return Err(ApplyError::InternalError(String::from("Cannot serialize record type container")))
        };
        self.context.set_state(&address, serialized.as_ref())
            .map_err(|err|ApplyError::InternalError(format!("{}", err)))?;
        Ok(())
    ...
    pub fn get_agent(&mut self, agent_id: &str) -> Result<Option<agent::Agent>, ApplyError> {
        ...
        match d {
            Some(packed) => {
                let agents: agent::AgentContainer = match protobuf::parse_from_bytes(packed.as_slice()) {
                    Ok(agents) => agents,
                    Err(_) => return Err(ApplyError::InternalError(String::from("Cannot deserialize agent container")))
                };
    ...
    pub fn set_agent(&mut self, agent_id: &str, agent: agent::Agent) -> Result<(), ApplyError> {
        ...
        let serialized = match agents.write_to_bytes() {
            Ok(serialized) => serialized,
            Err(_) => return Err(ApplyError::InternalError(String::from("Cannot serialize agent container")))
        };
        self.context.set_state(&&address, serialized.as_ref())
            .map_err(|err|ApplyError::InternalError(format!("{}", err)))?;
        Ok(())
    ...
    pub fn get_property(&mut self, record_id: &str, property_name: &str) -> Result<Option<property::Property>, ApplyError> {
        let address = make_property_address(record_id, property_name, 0);
        let d = self.context.get_state(&address)?;
        match d {
            Some(packed) => {
                let properties: property::PropertyContainer = match protobuf::parse_from_bytes(packed.as_slice()) {
                    Ok(properties) => properties,
                    Err(_) => return Err(ApplyError::InternalError(String::from("Cannot deserialize property container")))
                };
    ...
    pub fn set_property(&mut self, record_id: &str, property_name: &str, property: property::Property) -> Result<(), ApplyError> {
        let address = make_property_address(record_id, property_name, 0);
        let d = self.context.get_state(&address)?;
        ...
        let serialized = match property_container.write_to_bytes() {
            Ok(serialized) => serialized,
            Err(_) => return Err(ApplyError::InternalError(String::from("Cannot serialize property container")))
        };
        self.context.set_state(&address, serialized.as_ref())
            .map_err(|err|ApplyError::InternalError(format!("{}", err)))?;
        Ok(())
    ...
    pub fn get_property_page(&mut self, record_id: &str, property_name: &str, page: u32) -> Result<Option<property::PropertyPage>, ApplyError> {
        let address = make_property_address(record_id, property_name, page);
        let d = self.context.get_state(&address)?;
        match d {
            Some(packed) => {
                let property_pages: property::PropertyPageContainer = match protobuf::parse_from_bytes(packed.as_slice()) {
                    Ok(property_pages) => property_pages,
                    Err(_) => return Err(ApplyError::InternalError(String::from("Cannot deserialize property page container")))
                };
    ...
    pub fn set_property_page(&mut self, record_id: &str, property_name: &str, page_num: u32,
            property_page: property::PropertyPage) -> Result<(), ApplyError> {
        ...
        let serialized = match property_pages.write_to_bytes() {
            Ok(serialized) => serialized,
            Err(_) => return Err(ApplyError::InternalError(String::from("Cannot serialize property page container")))
        };
        self.context.set_state(&address, serialized.as_ref())
            .map_err(|err|ApplyError::InternalError(format!("{}", err)))?;
        Ok(())
    ...
    pub fn get_proposal_container(&mut self, record_id: &str, agent_id: &str)
            -> Result<Option<proposal::ProposalContainer>, ApplyError> {
        let address = make_proposal_address(record_id, agent_id);
        let d = self.context.get_state(&address)?;
        match d {
            Some(packed) => {
                let proposals: proposal::ProposalContainer = match protobuf::parse_from_bytes(packed.as_slice()) {
                    Ok(property_pages) => property_pages,
                    Err(_) => return Err(ApplyError::InternalError(String::from("Cannot deserialize proposal container")))
                };
    ...
    pub fn set_proposal_container(&mut self, record_id: &str, agent_id: &str, proposals: proposal::ProposalContainer)
            -> Result<(), ApplyError> {
        let address = make_proposal_address(record_id, agent_id);
        let serialized = match proposals.write_to_bytes() {
            Ok(serialized) => serialized,
            Err(_) => return Err(ApplyError::InternalError(String::from("Cannot serialize proposal container")))
        };
        self.context.set_state(&address, serialized.as_ref())
            .map_err(|err|ApplyError::InternalError(format!("{}", err)))?;
        Ok(())
...
impl SupplyChainTransactionHandler {
    ...
    fn _create_agent(&self, payload: payload::CreateAgentAction, mut state: SupplyChainState,
                signer: &str, timestamp: u64) ->  Result<(), ApplyError> {
        let name = payload.get_name();
        match state.get_agent(signer) {
            Ok(Some(_)) => return Err(ApplyError::InvalidTransaction(
    ...
    fn _create_record(&self, payload: payload::CreateRecordAction, mut state: SupplyChainState,
                signer: &str, timestamp: u64) ->  Result<(), ApplyError> {
        match state.get_agent(signer){
            Ok(Some(_)) => (),
            Ok(None) => return Err(ApplyError::InvalidTransaction(
                format!("Agent is not register: {}", signer))),
            Err(err) => return Err(err)
        }
        ...
        let record_type = match state.get_record_type(type_name) {
            ...
            Ok(None) => return Err(ApplyError::InvalidTransaction(
                format!("Record Type does not exist {}", type_name))),
            Err(err) => return Err(err)
        };
        ...
        for name in required_properties.keys() {
            if !provided_properties.contains_key(name) {
                return Err(ApplyError::InvalidTransaction(
                        format!("Required property {} not provided", name)))
            }
        }
        ...
        for (provided_name, provided_properties) in provided_properties.clone() {
            let required_type = match type_schemata.get(provided_name) {
                ...
                None => return Err(ApplyError::InvalidTransaction(
                    format!("Provided property {} is not in schemata", provided_name)))
            };
            let provided_type = provided_properties.data_type;
            if provided_type != required_type {
                return Err(ApplyError::InvalidTransaction(
                        format!("Value provided for {} is the wrong type", provided_name)))
            };
    ...
    fn _finalize_record(&self, payload: payload::FinalizeRecordAction, mut state: SupplyChainState,
                signer: &str) ->  Result<(), ApplyError> {
        let record_id = payload.get_record_id();
        ...
        let owner = match final_record.owners.last() {
            Some(x) => x,
            None => return Err(ApplyError::InvalidTransaction(
                String::from("Owner was not found"))),
        };
        let custodian = match final_record.custodians.last() {
            Some(x) => x,
            None => return Err(ApplyError::InvalidTransaction(
                String::from("Custodian was not found"))),
        };
    ...
    fn _create_record_type(&self, payload: payload::CreateRecordTypeAction, mut state: SupplyChainState,
                signer: &str) ->  Result<(), ApplyError> {
        match state.get_agent(signer){
            Ok(Some(_)) => (),
            Ok(None) => return Err(ApplyError::InvalidTransaction(
        ...
        match state.get_record_type(name) {
            Ok(Some(_)) => return Err(ApplyError::InvalidTransaction(
              format!("Record type already exists: {}", signer))),
            Ok(None) => (),
            Err(err) => return Err(err)
        }
    ...
    fn _update_properties(&self, payload: payload::UpdatePropertiesAction, mut state: SupplyChainState,
                signer: &str, timestamp: u64) ->  Result<(), ApplyError> {
        let record_id = payload.get_record_id();
    ...
    fn _create_proposal(&self, payload: payload::CreateProposalAction, mut state: SupplyChainState,
                signer: &str, timestamp: u64) ->  Result<(), ApplyError> {
        ...
        for prop in open_proposals {
            if prop.get_receiving_agent() == receiving_agent && prop.get_role() == role
                ...
                return Err(ApplyError::InvalidTransaction(String::from("Proposal already exists")))
            }
        }
        ...
        proposals.entries.sort_by_key(|p| (p.clone().record_id, p.clone().receiving_agent,
            p.clone().timestamp));
    ...
    fn _answer_proposal(&self, payload: payload::AnswerProposalAction, mut state: SupplyChainState,
            signer: &str, timestamp: u64) ->  Result<(), ApplyError> {
        ...
        match response {
            payload::AnswerProposalAction_Response::CANCEL => {
                if current_proposal.get_issuing_agent() != signer {
                    return Err(ApplyError::InvalidTransaction(
                        String::from("Only the issuing agent can cancel a proposal")))
                }
            ...
            payload::AnswerProposalAction_Response::REJECT => {
                if current_proposal.get_receiving_agent() != signer {
                    return Err(ApplyError::InvalidTransaction(
                        String::from("Only the receiving agent can reject a proposal")))
                }
            ...
            payload::AnswerProposalAction_Response::ACCEPT => {
                if current_proposal.get_receiving_agent() != signer {
                    return Err(ApplyError::InvalidTransaction(
                        String::from("Only the receiving agent can Accept a proposal")))
                };
                ...
                match role {
                    proposal::Proposal_Role::OWNER => {
                        if owner.get_agent_id() != current_proposal.get_issuing_agent() {
                            ...
                            proposals.entries.sort_by_key(|p| (p.clone().record_id,
                                p.clone().receiving_agent, p.clone().timestamp));
                            state.set_proposal_container(&record_id, &receiving_agent, proposals)?;
                            return Ok(())
                        }
                        ...
                        for prop_schema in record_type.get_properties() {
                            let mut prop = match state.get_property(record_id, prop_schema.get_name()){
                                Ok(Some(prop)) => prop,
                                Ok(None) => return Err(ApplyError::InvalidTransaction(
                                      String::from("Property does not exist"))),
                                Err(err)=> return Err(err)
                            };
                    ...
                    proposal::Proposal_Role::CUSTODIAN => {
                        if custodian.get_agent_id() != current_proposal.get_issuing_agent() {
                            current_proposal.status = proposal::Proposal_Status::CANCELED;
                            info!("Record custodian does not match the issuing agent of the proposal");
                            // remove old proposal and replace with new one
                            proposals.entries.remove(proposal_index);
                            proposals.entries.push(current_proposal.clone());
                            proposals.entries.sort_by_key(|p| (p.clone().record_id,
                                p.clone().receiving_agent, p.clone().timestamp));
                            state.set_proposal_container(&record_id, &receiving_agent, proposals.clone())?;
                        }
    ...
    fn _revoke_reporter(&self, payload: payload::RevokeReporterAction, mut state: SupplyChainState,
            signer: &str) ->  Result<(), ApplyError> {
        let record_id = payload.get_record_id();
        ...
        if owner.get_agent_id() != signer{
            return Err(ApplyError::InvalidTransaction(
                format!("Must be owner to revoke reporters")))
        }
        ...
        for prop_name in properties {
            let mut prop = match state.get_property(record_id, prop_name) {
                Ok(Some(prop)) => prop,
                Ok(None) => return Err(ApplyError::InvalidTransaction(
                    format!("Property does not exists"))),
                Err(err) => return Err(err)
            };
            ...
            for reporter in prop.get_reporters() {
                if reporter.get_public_key() == reporter_id {
                    if !reporter.get_authorized(){
                        return Err(ApplyError::InvalidTransaction(
                            format!("Reporter is already unauthorized.")))
                    }
    ...
    fn _make_new_reported_value(&self, reporter_index: u32, timestamp: u64,
            prop: &property::PropertyValue) -> property::PropertyPage_ReportedValue {
    ...
    fn apply(&self, request: &TpProcessRequest, context: &mut TransactionContext) -> Result<(), ApplyError> {
        ...
        match payload.get_action() {
            ...
            Action::UpdateProperties(update_properties_payload) => {
                self._update_properties(update_properties_payload, state, signer, payload.get_timestamp())?
            },
            ...
            Action::AnswerProposal(answer_proposal_payload) => {
                self._answer_proposal(answer_proposal_payload, state, signer, payload.get_timestamp())?
            },
